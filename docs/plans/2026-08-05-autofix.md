...
> **Historical record — implemented, and the code has moved since.** This is
> the plan as written before the work, kept for the reasoning behind each
> task, not as a description of what shipped. The code snippets below are
> stale in at least these ways: `_gh` catches `OSError` rather than
> `FileNotFoundError` and scrubs an inherited `GH_TOKEN`; `find_open_issue`
> lists on the label instead of searching; `ensure_labels` does not pass
> `--force`; `file_bug` refunds a dispatch when `issue create` fails; and
> `AutofixConfig` gained `report_cooldown_s`, which the plan never
> anticipated. Read `src/gpuqueue/bugfiler.py` for what is true.
